...
class Agent():
    def __init__(self):
        self.policy = NNPolicy(1, 256, 3)
        self.hx = torch.zeros(1, 256)
        # self.policy.try_load(load_model="final_model.pth")
        self.load_model()

    def load_model(self, fname=None):
        if fname is None:
            self.policy.load_state_dict(torch.load("final_model.pth", map_location="cpu"))
        else:
            self.policy.load_state_dict(torch.load(fname, map_location="cpu"))
    ...
    def get_action(self, state):
        ...
        _, logit, hx = self.policy((state.view(1,1,80,80), self.hx))
...
class NNPolicy(nn.Module): # an actor-critic neural network
    ...
    def try_load(self, save_dir=None, load_model=None):
