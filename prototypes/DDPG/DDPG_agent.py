...
class DDPG_Agent:
    def __init__(self, ob_sp, act_sp, alow, ahigh, writer):
        self.alow = alow
        self.ahigh = ahigh
    ...
    def train(self):
        ...
        #  CRITIC LOSS
        # inputs computation
        inputs_loss = self.qnet(states, actions)
        # targets
        with torch.no_grad():
            policy_acts = self.policy_targ(next_states)
        targ_values = self.qnet_targ(next_states, policy_acts)
        targets_loss = rewards + GAMMA * (1 - dones) * targ_values
        loss_critic = ((1 / 2) * (inputs_loss - targets_loss)**2).mean()
